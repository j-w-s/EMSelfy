...
The original Electron version distributed specific Windows .dll files. These are not included in the repository. Both the Tauri and Electron versions work on Linux (as this is what I use and have tested), but I haven't tested on other systems. I assume without the .dll files the Electron version may crash or fail to load on Windows. Since this requires assets from the original archive, you could just copy them over if necessary. Regardless, you will need from it: `selfy.asar`, `selfy.db`. Drop these into either `emselfy-electron/` or `emselfy-tauri/`. Needless to say, this distribution issue prevents me from setting up Github Actions, and I wouldn't dare do so anyway due to copyright.

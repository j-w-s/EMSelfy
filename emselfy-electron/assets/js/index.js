...
$(window).on("init", function () {
    ...
    (function () {
        ...
        $(".panel_toggle_btn").on("click", function () {
            var $this = $(this);
            var panel = $this.data("panel");
            $(".panel_toggle_btn.active")
            .removeClass("active");
    ...
    (function () {
        ...
        var div = $(".l-selfy"), rpanel = $(".r-panel");
        ...
        win.on("resize", function () {
            ...
            div.css(flash_size); $(".selfy_flash").attr(flash_size);
            rpanel.css({ "margin-left": flash_size.width + 44, "width": win_size.width - flash_size.width - 44 - 2 });
        });
    })();
    ...
    (function () {
        ...
        window.set_clothes = function (category, id, auto) {
            ...
            if (category == "clear") {
                ...
                item.data("id", id);
                if (id) { $("img", item).attr("src", window.serverurl + "/imgs/" + category + "/" + id + ".png"); }
                else { $("img", item).attr("src", ""); }
                
                var exist = false;
                if (!id) {
                    for (var i in selfy_history) {
                        var item = selfy_history[i];
                        if (item == id) { exist = true; break; }
                    };
                }
                if (!exist) { selfy_history.push(id); }
            }
            if (auto) {
                update_clothes();
            }
        ...
        window.update_clothes = function () {
            ...
            $.post(window.serverurl + "/history?" + lib.qs.stringify({ action: "add", ids: selfy_history }));
            selfy_history = [];
        ...
        $(".l-selfy .selfy-li .seach").on("click", function () {
            ...
            if ($this.data("id")) { str += "id:" + $this.data("id") + " "; }
            $(".e_seach_input").val(str)
            .parents("form:eq(0)")
            .trigger("submit");
        });
    ...
    (function () {
        var seach_key = $(".s-input input"), seach_input = seach_key.parents(".s-b-key:eq(0)");
        var more = $(".s-b-key .s-ico"), more_box = $(".s-b-key .s-b-more");
        
        seach_key.on("focus context focusout", function (e) {
            if (e.type == "context") {
                if (this.value.trim() == "") { seach_input.removeClass("focus"); }
                else { seach_input.addClass("focus"); }
                return;
            }
        ...
        more.on("click", function () {
            ...
            for (var key in objs) {
                ...
                .attr("value", objs[key]).prop("selected", true);
        ...
        var more = $(".e_seach_more"), input = $(".e_seach_input");
        
        var updatekey = function () {
            var qs = {};
            $("input,select", more).each(function (i, v) { v.name && (qs[v.name] = v.value); });
        ...
        $(".e_color").combobox("getinput").autocomplete("instance")._renderItem = function (ul, item) {
            return $("<li>")
            .css("color", item.value)
            .text(item.label)
            .appendTo(ul);
        };
        ...
        .on("enter", function () {
            var $this = $(this);
            var input = $this.data("input"), params = $this.data("params"), onequery = $this.data("onequery");
            input = input || "";
        ...
        $(".e_seach_btn").on("click", function () { $(".e_seach_form").trigger("submit"); });
        var view_btn = $(".e_view_btn").on("click", function () { view_box.toggleClass("active"); });
        $(".s-l-v .btn-text").on("click", function () {
            var $this = $(this);
            $("i", view_btn).attr("class", $("i", $this).attr("class"));
            view_box.removeClass("active");
            
            if ($this.hasClass("max")) { $(".l-s").removeClass("min"); $(".l-s").addClass("max"); }
            if ($this.hasClass("min")) { $(".l-s").removeClass("max"); $(".l-s").addClass("min"); }
            win.trigger("resize");
        });
        
        var tp_item = baidu.template("tp:selfy_item"), view_box = $(".s-l-v");
        ...
        var event_list = function (html, params) {
            var b = $(".selfy_items .l-s > ." + params.query.type)
                .data("params", params);
            ...
            if (localStorage["loadtype"] == "1") {
                ...
                var go_page = function (old, p) {
                    ...
                    if (old.query.pn == q_pn) { return; }
                    old.query.pn = q_pn;
                    query(old.query, event_list);
                }
                
                $(".page-bottom .paging span[data-p]", b).on("click", function () {
                    var old = $(this).parents("." + params.query.type).data("params");
                    go_page(old, $(this).data("p"));
                });
                $(".page-bottom .paging .input input", b).on("click", function () { return false; })
                ...
                if (params.query.pn == 0) { b.html(html); }
                else { b.append(html); }
            }
            ...
            var effect_box_to = function (from, to , data) {
                ...
                var from_off = from.offset(), to_off = to.offset();
                var e = $("<div>")
                .html(html)
            ...
            $(".l-s-item .wear", b).on("click", function () {
                ...
                effect_box_to($("img", $this), ".l-selfy .selfy-li." + $this.data("category") + " img", { img: $("img", $this).attr("src") });
            ...
            $(".l-s-item .favorites", b).on("click", function () {
                ...
                if ($this.data("favorites")) {
                    $.get(window.serverurl + "/favorites", { action: "del", id: $this.data("id") }, function (msg) {
                        $this.data("favorites", !$this.data("favorites"));
                        ico.attr("class", "iconfont icon-Collection");
                    }, "json");
                } else {
                    $.get(window.serverurl + "/favorites", { action: "add", id: $this.data("id") }, function (msg) {
                        effect_box_to($("img", $this), ".l-bar li[data-panel=favorites]", { img: $("img", $this).attr("src") });
                        $this.data("favorites", !$this.data("favorites"));
        ...
        $(".selfy_items .l-s").on("scroll", function (e) {
            if (localStorage["loadtype"] != "2") { return; }
            var $this = $(this),
                panel = $(".selfy_items .l-s .active.panel_toggle_p"),
                falls = $(".falls", panel);
            
            if (panel.data("nodate") || panel.data("isloading")) { return; }
        ...
        $(".e_seach_form").on("submit", function (e) {
            e.preventDefault();
            more_box.stop().slideUp(100);
            var $this = $(this);
    ...
    (function () {
        ...
        var rpanel = $(".r-panel"), seach = $(".s-seach", rpanel), ls = $(".selfy_items .l-s", rpanel);
    ...
    (function () {
        ...
        $(".file .e_img_import").on("click", function () {
            ipc.send("openDialog" , "open", {
                properties: ['openFile']
                ,filters: [{ name: 'em selfy', extensions: ['emselfy'] }]
            });
        });
        
        ipc.on("openDialog-back", function (e, type, path) {
            if (path == null || path.length < 1) return;
            switch (type) {
                case "open":
                    lib.fs.readFile(path[0], "utf8", function (err, data) {
                        if (err) { return; }
                        window.set_clothes_url(data);
                    });
                    break;
                case "save":
                    lib.fs.writeFile(path, window.selfy_query, "utf8");
                    break;
                default:
            }
        ...
        //$(".file .e_img_export").on("click", function () {
        //    ipc.send("openPreview" , window.selfy_query + "&action=shot"); //最小化窗口
        //});
        ...
        $(".file .e_to_atgame").on("click", function () {
            var url = "http://img.atgames.jp/selfy_motion.swf?" + window.selfy_query + "&_from=emselfy&_ver=1.0";
            console.log(url);
            utils.openURL(url);
        });

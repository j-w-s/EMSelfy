...
exports.start = function (p) {
    ...
    var runAppInit = function () {
        wins['main'].webContents.executeJavaScript(
            "window.winid='main';window.localprot='" +
                prot +
    ...
    if (wins['main'].webContents.isLoading()) {
        wins['main'].webContents.once('did-finish-load', runAppInit);
    } else {
        runAppInit();
    }
};
...
electron.ipcMain.on('openPreview', function (e, params) {
    ...
    wins['preview'].webContents.on('did-finish-load', function () {
        ...
        wins['preview'].webContents.executeJavaScript(
            "window.winid='preview';window.localprot='" +
                prot +
                "';window.serverurl='http://localhost:" +
                prot +
                "';",

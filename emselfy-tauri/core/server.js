...
exports.init = function (p) {
    ...
    setImmediate(function () {
        try { lib.asar.exists(lib.path.join(__dirname, "../", config["selfy-file"]), "/imgs"); }
        catch (e) { }
    });
...
var getInfo = function (url, query, request, response) {
    ...
    try {
        var sources = $u.array.select(infoDb.query(source_sql).all(), function (v) { return v["value"]; });
        var styles = $u.array.select(infoDb.query(style_sql).all(), function (v) { return v["style"]; });
...
var getClothes = function (url, query, request, response) {
    ...
    if (lib.asar.exists(archivePath, url.pathname)) {
        ...
        response.writeHead(200, { "content-type": getContentType(url.pathname), "content-length": asarBuf.length });
    ...
    if (lib.fs.existsSync(fsPath)) {
        lib.fs.readFile(fsPath, undefined, function (err, b) {
            ...
            response.writeHead(200, { "content-type": getContentType(url.pathname), "content-length": b.length });
            response.write(b);
            response.end();
        });
        return;

...
_obj.prototype = {
    ...
    , _initDB: function (callback) {
        var sql = "CREATE TABLE IF NOT EXISTS [EMSelfy_userFavorites] (         \n" +
    ...
    , addFavorites: function (ids, callback) {
        ...
        this.delFavorites(ids, function () {
            ...
            sql += lib.$u.array.select(ids, function (item) {
                return lib.str.format("('{0}','{1}','{2}')", lib.guid(), item, lib.str.timeFormat(new Date(), "yyyy-MM-dd HH:mm:ss"))
            }).join(",");

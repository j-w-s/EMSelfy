...
var _obj = function (a) {
    ...
    // bun:sqlite is synchronous; wrap for callback-style compat
    var self = this;
    this._db.all = function(sql, params, cb) {
        try {
            var rows = self._db.query(sql).all(params || {});
            if (cb) cb(null, rows);
        } catch(e) {
            if (cb) cb(e, []);
        }
    ...
    this._db.serialize = function(fn) { if (fn) fn(); };
}
...
_obj.prototype = {
    ...
    ,getClohesInfo: function (query, callback) {
        ...
        lib.$u.each(['face', 'hat', 'hair', 'accef', 'top', 'coat', 'back', 'bottom', 'accen', 'bg', 'shoe', 'fg', 'ffg', 'acceh', 'motion'], function (name) {
        ...
        if (selfy_ids.length == 0) { lib.$func.apply(callback, this, undefined, []); return; }
        
        var sql = "SELECT [Id],[ClothesId],[Category],[Part],[Sort],[CreateTime] FROM [Selfy_Part] where [ClothesId] in ({0})";
    ...
    ,getList: function (query, callback) {
        ...
        var query_db = function (sql1, sql_params1, sql2, sql_params2, config) {
            $this._db.all(sql1, sql_params1, function (err, rows) {
                if (err) { lib.$func.apply(callback, $this, [err, {}]); return; }
                $this._db.all(sql2, sql_params2, function (err, count) {
                    if (err) { lib.$func.apply(callback, $this, [err, {}]); return; }
                    
                    var sql = "select [ClothesId] from [EmSelfy_userFavorites] where [ClothesId] in('{0}')";
                    sql = lib.str.format(sql,
                        lib.$u.array.select(rows, function (q) { return q.Id }).join("','")
                    );
                    $this._db.all(sql, {}, function (err, farow) {
                        if (err) { lib.$func.apply(callback, $this, [err, []]); return; }
                        rows = lib.$u.array.select(rows, function (item) {
                            item.favorites = lib.$u.array.where(farow, function (q) { return q.ClothesId == item.Id; }).length
                            return item;
                        });
                        
                        lib.$func.apply(callback, $this, [err, { count: count[0]['c'], pagesize: config.pageSize, data: rows, }]);
                    });
